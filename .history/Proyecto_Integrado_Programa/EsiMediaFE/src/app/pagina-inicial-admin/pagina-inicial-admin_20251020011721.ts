...
export class PaginaInicialAdmin implements OnInit, OnDestroy {
  ...
  constructor(private readonly api: AuthService, private readonly auth: AuthService, private readonly router: Router) {
    this.searchChanged$.pipe(debounceTime(250), takeUntil(this.destroy$)).subscribe(() => { /* fuerza reevaluación */ void this.filteredUsers; });
    this.aliasCheck$.pipe(debounceTime(250), takeUntil(this.destroy$)).subscribe(v => {
      const id = this.editingUser?.id;
      const exists = !!this.users.find(u => (u.alias||'').toLowerCase()===trimLower(v) && u.id!==id);
    ...
    this.adminAliasCheck$.pipe(debounceTime(250), takeUntil(this.destroy$)).subscribe(v => {
      ...
      const exists = !!this.users.find(u => (u.alias||'').toLowerCase()===trimLower(v) && u.id!==id);
      this.adminAliasTaken = exists;
      const r = validateAdminFormModel(this.adminEditModel, this.adminAliasTaken);
      this.adminFieldErrors = r.errors; this.isAdminFormValid = r.valid;
    });
  }
  ...
  onAdminApellidosInput(v: string){ (this.adminEditModel.apellidos as any)=v; this.applyAdminValidation(); }
  onAdminDepartamentoInput(v: string){ (this.adminEditModel.departamento as any)=v; this.applyAdminValidation(); }
  onAdminNameInput(v: string){ (this.adminEditModel.nombre as any)=v; this.applyAdminValidation(); }
  onAdminEmailInput(v: string){ (this.adminEditModel.email as any)=v; this.applyAdminValidation(); }
  private applyAdminValidation(){ const r=validateAdminFormModel(this.adminEditModel,this.adminAliasTaken); this.adminFieldErrors=r.errors; this.isAdminFormValid=r.valid; }
  ...
  confirmAction() {
    ...
    obs.subscribe({
      ...
      error: err => { this.loading=false; this.errorMsg=err?.error?.message||'Operación no realizada'; this.cancelConfirm(); }
    });
  }
  ...
  createAdmin(form: NgForm) {
    ...
    const b:any = {
      nombre:trimSafe(this.crearAdmin.nombre), apellidos:trimSafe(this.crearAdmin.apellidos),
      alias:trimSafe(this.crearAdmin.alias), email:trimSafe(this.crearAdmin.email),
