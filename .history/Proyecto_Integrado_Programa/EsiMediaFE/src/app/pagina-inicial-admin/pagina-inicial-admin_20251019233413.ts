...
import { Subject, debounceTime, takeUntil, throwIfEmpty } from 'rxjs';
...
export class PaginaInicialAdmin implements OnInit, OnDestroy {
  ...
  onFechaNacInput(value: string) {
    ...
    const hoy = new Date();
    if (fecha > hoy) {
      this.fechaNacError = '⚠️ La fecha no puede ser futura.';
      return;
    }
    ...
    if (edadReal < 4) {
      this.fechaNacError = '⚠️ El usuario debe tener al menos 4 años.';
    }
  }
