...
export class PaginaInicialAdmin implements OnInit, OnDestroy {
  ...
  saveEdit(form: NgForm) {
    ...
    this.validateEditForm();
    if (form.invalid || this.aliasTaken || !this.isEditFormValid) return;
  ...
  private validateEditForm() {
    ...
    const fn = (this.editModel.fechaNac ?? '')?.trim();
    if (al) {
      if (al.length < this.ALIAS_MIN || al.length > this.MAX.alias) errors['alias'] = `El alias debe tener entre ${this.ALIAS_MIN} y ${this.MAX.alias} caracteres.`;
      else if (this.aliasTaken) errors['alias'] = 'El alias ya está en uso.';
    }
    if (n && n.length > this.MAX.nombre) errors['nombre'] = `El nombre supera ${this.MAX.nombre} caracteres.`;
    if (ap && ap.length > this.MAX.apellidos) errors['apellidos'] = `Los apellidos superan ${this.MAX.apellidos} caracteres.`;
    if (this.editingUser?.role === 'GESTOR_CONTENIDO') {
      if (de && de.length > this.MAX.descripcion) errors['descripcion'] = `La descripción supera ${this.MAX.descripcion} caracteres.`;
      if (es && es.length > this.MAX.especialidad) errors['especialidad'] = `La especialidad supera ${this.MAX.especialidad} caracteres.`;
    }
    if (this.editingUser?.role === 'USUARIO') {
      if (!fn) errors['fechaNac'] = 'La fecha de nacimiento es obligatoria.';
      else {
        const today = new Date();
        const d = new Date(fn);
        if (d > today) errors['fechaNac'] = 'La fecha no puede ser futura.';
      }
    }
    this.editFieldErrors = errors;
    this.isEditFormValid = Object.keys(errors).length === 0;
  }
