...
export class PaginaInicialAdmin implements OnInit, OnDestroy {
  ...
  private applyAdminValidation() {
    const errors: Record<string, string> = {};
    const { nombre, apellidos, email, departamento } = this.adminEditModel;

    if (!nombre.trim()) errors['nombre'] = 'El nombre es obligatorio.';
    if (!apellidos.trim()) errors['apellidos'] = 'Los apellidos son obligatorios.';
    if (!email.trim() || !EMAIL_RE.test(email)) errors['email'] = 'El email no es válido.';
    if (departamento.length > 120) errors['departamento'] = 'El departamento supera 120 caracteres.';

    this.adminFieldErrors = errors;
    this.isAdminFormValid = Object.keys(errors).length === 0;
  }
  cancelEditAdmin() { this.showEditAdminModal=false; this.editingAdmin=null; }
