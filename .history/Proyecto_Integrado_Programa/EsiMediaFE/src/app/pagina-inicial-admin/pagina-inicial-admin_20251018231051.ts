...
onAdminDepartamentoInput(v: string) {
  ...
  const obs = this.getConfirmObservable(u, this.confirmKind);
  if (!obs) { this.loading = false; return; }

  obs.subscribe({
    next: (res: Partial<AppUser> | void) => this.handleConfirmSuccess(u, this.confirmKind!, res),
    error: (err: any) => this.handleConfirmError(err),
  });
}

private getConfirmObservable(u: AppUser, kind: ConfirmKind) {
  if (this.isCreator(u)) return this.pickCreatorAction(kind, u.id);
  if (this.isAdmin(u))   return this.pickAdminAction(kind, u.id);
  if (this.isUser(u))    return this.pickUserAction(kind, u.id);
  return null;
}

private pickCreatorAction(kind: ConfirmKind, id: string) {
  if (kind === 'block')   return this.api.blockCreator(id);
  if (kind === 'unblock') return this.api.unblockCreator(id);
  return this.api.deleteCreator(id);
}

private pickAdminAction(kind: ConfirmKind, id: string) {
  if (kind === 'block')   return this.api.blockAdmin(id);
  if (kind === 'unblock') return this.api.unblockAdmin(id);
  return this.api.deleteAdmin(id);
}

private pickUserAction(kind: ConfirmKind, id: string) {
  if (kind === 'block')   return this.api.blockUser(id);
  if (kind === 'unblock') return this.api.unblockUser(id);
  return this.api.deleteUser(id);
}

private handleConfirmSuccess(u: AppUser, kind: ConfirmKind, res: Partial<AppUser> | void) {
  if (kind === 'delete') {
...
private handleConfirmError(err: any) {
  this.loading = false;
