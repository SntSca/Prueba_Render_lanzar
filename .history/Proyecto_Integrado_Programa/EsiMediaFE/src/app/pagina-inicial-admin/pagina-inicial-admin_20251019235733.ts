...
export class PaginaInicialAdmin implements OnInit, OnDestroy {
  ...
  private filterByBlocked(data: AppUser[], b: EstadoFiltro) {
    if (b === '') {
      return data;
    }
    const w = b === 'si';
    return data.filter(u => !!u.blocked === w);
  }
  private filterByTipo(data: AppUser[], t: TipoFiltro) { return t ? data.filter(u => u.role === t) : data; }
  private filterByVip(data: AppUser[], v: VipFiltro) {
    if (v === '') {
      return data;
    }
    const w = v === 'si';
    return data.filter(u => u.role === 'USUARIO' && (!!u.vip === w));
  }
  ...
  private getEditObservable(u: AppUser, dto: any) {
    if (u.role === 'GESTOR_CONTENIDO') {
      return this.api.updateCreator(u.id, dto);
    } else if (u.role === 'USUARIO') {
      return this.api.updateUser(u.id, dto);
    } else {
      return null;
    }
  }
