...
const trimSafe = (s: string | null | undefined) => (s ?? '').trim();
const trimLower = (s: string | null | undefined) => trimSafe(s).toLowerCase();
const withinLen = (v: string, min: number, max: number) => v.length >= min && v.length <= max;
const overMax = (v: string | null | undefined, max: number) => !!(v ?? '') && (v as string).length > max;
const toDateInput = (iso?: string | null) => (!iso ? '' : iso.length > 10 ? iso.slice(0, 10) : iso);
const initials = (nombre: string) => {
  const s = trimSafe(nombre);
  return s ? s.split(/\s+/).map(p => p[0]).join('').toUpperCase() : 'U';
};
const calcEdad = (fechaISO: string) => {
...
export class PaginaInicialAdmin implements OnInit, OnDestroy {
  ...
  constructor(private readonly api: AuthService, private readonly auth: AuthService, private readonly router: Router) {
    ...
    this.aliasCheck$.pipe(debounceTime(250), takeUntil(this.destroy$)).subscribe(v => {
      const a = trimLower(v); const id = this.editingUser?.id;
    ...
    this.adminAliasCheck$.pipe(debounceTime(250), takeUntil(this.destroy$)).subscribe(v => {
      const a = trimLower(v); const id = this.editingAdmin?.id;
  ...
  private setLoggedUser(user: UserDto | null) {
    ...
    this.userInitials = initials(this.userName);
  ...
  private filterByQuery(d: AppUser[], q: string) {
    if (!q) return d;
    const t = q.toLowerCase();
    return d.filter(u => [u.alias,u.nombre,u.apellidos,u.email].some(x => (x||'').toLowerCase().includes(t)));
  }
  private filterByBlocked(d: AppUser[], b: EstadoFiltro) {
    if (b === '') return d;
    const w = b === 'si';
    return d.filter(u => !!u.blocked === w);
  }
  private filterByTipo(d: AppUser[], t: TipoFiltro) { return t ? d.filter(u => u.role === t) : d; }
  private filterByVip(d: AppUser[], v: VipFiltro) {
    if (v === '') return d;
    const w = v === 'si';
    return d.filter(u => u.role === 'USUARIO' && (!!u.vip === w));
  }
  ...
  get filteredUsers(): AppUser[] {
    ...
    d = this.filterByQuery(d, trimSafe(f.nombre));
  ...
  private applyRoleSpecific(u: AppUser, dto: any) {
    if (u.role==='GESTOR_CONTENIDO') {
      this.addIfChanged(dto,'descripcion',trimSafe(this.editModel.descripcion), (u as any).descripcion??'');
      this.addIfChanged(dto,'especialidad',trimSafe(this.editModel.especialidad), (u as any).especialidad??'');
      return;
    }
    if (u.role==='USUARIO') {
      const fn=trimSafe(this.editModel.fechaNac);
      if (fn) dto.fechaNac=fn;
    }
  }
  private buildEditDto(u: AppUser) {
    const m=this.editModel, dto:any={};
    this.addIfChanged(dto,'alias',trimSafe(m.alias),u.alias??'');
    this.addIfChanged(dto,'nombre',trimSafe(m.nombre),u.nombre??'');
    this.addIfChanged(dto,'apellidos',trimSafe(m.apellidos),u.apellidos??'');
    this.addIfChanged(dto,'email',trimSafe(m.email),u.email??'');
  ...
  private getEditObservable(u: AppUser, dto: any) {
    const m: Record<AppUser['role'], any> = {
      GESTOR_CONTENIDO: this.api.updateCreator(u.id, dto),
      USUARIO: this.api.updateUser(u.id, dto),
      ADMINISTRADOR: null
    };
    return m[u.role] ?? null;
  }
  ...
  saveEdit(form: NgForm) {
    ...
    const u=this.editingUser, dto=this.buildEditDto(u); if (!Object.keys(dto).length) { this.cancelEdit(); return; }
    this.loading=true; const obs=this.getEditObservable(u,dto); if (!obs) { this.loading=false; return; }
    obs.subscribe({ next: (upd: Partial<AppUser>) => this.onEditSuccessWithSwal(u,upd), error: (err: any) => this.onEditError(err) });
  }
  ...
  private onEditSuccess(u: AppUser, upd: unknown) {
    ...
    if (i >= 0) { this.users[i] = { ...(this.users[i]), ...(upd as any) }; this.loading = false; this.cancelEdit(); }
  }
  ...
  onFechaNacInput(value: string) {
    ...
    const {edad,futura}=calcEdad(value);
    this.fechaNacError = futura ? '⚠️ La fecha no puede ser futura.' : (edad<4 ? '⚠️ El usuario debe tener al menos 4 años.' : null);
  }
  ...
  private validateEditModel(u: AppUser): string | null {
    const alias=trimSafe(this.editModel.alias), nombre=trimSafe(this.editModel.nombre), apellidos=trimSafe(this.editModel.apellidos);
    if (alias && !withinLen(alias,this.ALIAS_MIN,this.MAX.alias)) return `El alias debe tener entre ${this.ALIAS_MIN} y ${this.MAX.alias} caracteres.`;
    if (overMax(nombre,this.MAX.nombre)) return `El nombre supera ${this.MAX.nombre} caracteres.`;
    if (overMax(apellidos,this.MAX.apellidos)) return `Los apellidos superan ${this.MAX.apellidos} caracteres.`;
    if (u.role!=='GESTOR_CONTENIDO') return null;
    const desc=trimSafe(this.editModel.descripcion), esp=trimSafe(this.editModel.especialidad);
    if (overMax(desc,this.MAX.descripcion)) return `La descripción supera ${this.MAX.descripcion} caracteres.`;
    if (overMax(esp,this.MAX.especialidad)) return `La especialidad supera ${this.MAX.especialidad} caracteres.`;
    return null;
  }
  ...
  validateAdminForm() {
    ...
    const f={ alias:trimSafe(this.adminEditModel.alias), nombre:trimSafe(this.adminEditModel.nombre), apellidos:trimSafe(this.adminEditModel.apellidos), email:trimSafe(this.adminEditModel.email), departamento:trimSafe(this.adminEditModel.departamento) };
    const e:Record<string,string> = {};
    if (f.alias) {
      if (!withinLen(f.alias, MIN, MAX.alias)) e['alias'] = `El alias debe tener entre ${MIN} y ${MAX.alias} caracteres.`;
      else if (this.adminAliasTaken) e['alias'] = 'El alias ya está en uso.';
    }
    if (!f.nombre) e['nombre']='El nombre es obligatorio.'; else if (overMax(f.nombre,MAX.nombre)) e['nombre']= `El nombre supera ${MAX.nombre} caracteres.`;
    if (overMax(f.apellidos,MAX.apellidos)) e['apellidos'] = `Los apellidos superan ${MAX.apellidos} caracteres.`;
    if (!f.email) e['email']='El email es obligatorio.'; else if (!EMAIL_RE.test(f.email)) e['email']='Email no válido.';
    if (overMax(f.departamento,MAX.departamento)) e['departamento'] = `El departamento supera ${MAX.departamento} caracteres.`;
  ...
  saveAdminEdit(form: NgForm) {
    ...
    const dto:any = {};
    const set = (k:keyof typeof this.adminEditModel, v:string|null) => { const nv = v===null?null:trimSafe(v||''); if (nv!== (u as any)[k]) dto[k] = nv; };
    set('nombre', this.adminEditModel.nombre);
    set('apellidos', this.adminEditModel.apellidos);
    set('email', this.adminEditModel.email);
    dto.foto = this.adminEditModel.foto??null;
    if (trimSafe(this.adminEditModel.departamento) !== trimSafe((u as any).departamento)) dto.departamento = trimSafe(this.adminEditModel.departamento);
  ...
  editUser(u: AppUser) {
    if (this.isSuperAdmin(u)) return;
    const actions: Record<AppUser['role'],()=>void> = {
      GESTOR_CONTENIDO: () => this.openEditModal(u),
      USUARIO: () => this.openEditModal(u),
      ADMINISTRADOR: () => this.openEditAdminModal(u)
    };
    actions[u.role]?.();
  }
  ...
  createAdmin(form: NgForm) {
    ...
    const b={ nombre:trimSafe(this.crearAdmin.nombre), apellidos:trimSafe(this.crearAdmin.apellidos), alias:trimSafe(this.crearAdmin.alias), email:trimSafe(this.crearAdmin.email), pwd:this.crearAdmin.pwd, pwd2:this.crearAdmin.pwd2, foto:trimSafe(this.crearAdmin.foto)||undefined, departamento:trimSafe(this.crearAdmin.departamento), fechaNac:trimSafe(this.crearAdmin.fechaNac)||undefined };
    this.loading=true;
