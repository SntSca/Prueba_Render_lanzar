...
export class PaginaInicialAdmin implements OnInit, OnDestroy {
  ...
  validateAdminForm() {
    ...
    if (f['alias']) {
      if (!PaginaInicialAdmin.withinLen(f['alias'], MIN, MAX.alias)) {
        e['alias'] = `El alias debe tener entre ${MIN} y ${MAX.alias} caracteres.`;
      }
      if (!e['alias'] && this.adminAliasTaken) {
        e['alias'] = 'El alias ya está en uso.';
      }
    }
