...
export class PaginaInicialAdmin implements OnInit, OnDestroy {
  ...
  constructor(private readonly api: AuthService, private readonly auth: AuthService, private readonly router: Router) {
    ...
    this.aliasCheck$.pipe(debounceTime(250), takeUntil(this.destroy$)).subscribe(v => {
      const a = PaginaInicialAdmin.trimLower(v); const id = this.editingUser?.id;
    ...
    this.adminAliasCheck$.pipe(debounceTime(250), takeUntil(this.destroy$)).subscribe(v => {
      const a = PaginaInicialAdmin.trimLower(v); const id = this.editingAdmin?.id;
  ...
  private filterByQuery(data: AppUser[], q: string) {
    if (!q) return data; const t=q.toLowerCase();
    return data.filter(u => [u.alias,u.nombre,u.apellidos,u.email].some(x => (x||'').toLowerCase().includes(t)));
  }
  ...
  get filteredUsers(): AppUser[] {
    ...
    d = this.filterByQuery(d, PaginaInicialAdmin.trimSafe(f.nombre));
  ...
  private applyRoleSpecific(u: AppUser, dto: any) {
    if (u.role==='GESTOR_CONTENIDO') {
      ...
      const fn=PaginaInicialAdmin.trimSafe(this.editModel.fechaNac); if (fn) dto.fechaNac=fn;
    }
  }
  ...
  private getEditObservable(u: AppUser, dto: any) { return u.role==='GESTOR_CONTENIDO' ? this.api.updateCreator(u.id,dto) : u.role==='USUARIO' ? this.api.updateUser(u.id,dto) : null; }
  ...
  private onEditSuccess(u: AppUser, upd: unknown) { const i=this.users.findIndex(x=>x.id===u.id); if (i>=0) this.users[i]={...(this.users[i]), ...(upd as any)}; this.loading=false; this.cancelEdit(); }
  ...
  validateAdminForm() {
    ...
    if (f.alias) { if (!PaginaInicialAdmin.withinLen(f.alias,MIN,MAX.alias)) e.alias=`El alias debe tener entre ${MIN} y ${MAX.alias} caracteres.`; if (!e.alias && this.adminAliasTaken) e.alias='El alias ya está en uso.'; }
  ...
  saveAdminEdit(form: NgForm) {
    ...
    const dto:any={}; if (nombre && nombre!==u.nombre) dto.nombre=nombre; if (apellidos!==(u.apellidos??'')) dto.apellidos=apellidos; if (email && email!==u.email) dto.email=email; dto.foto=foto; if (dep && dep!==((u as any).departamento??'')) dto.departamento=dep;
    if (!Object.keys(dto).length) { this.cancelEditAdmin(); return; }
    this.loading=true;
    this.api.updateAdmin(u.id,dto).subscribe({
      next: upd => { const i=this.users.findIndex(x=>x.id===u.id); if (i>=0) this.users[i]={...(this.users[i]), ...(upd as any)}; this.loading=false; this.cancelEditAdmin(); Swal.fire({ icon:'success', title:'¡Administrador actualizado!', text:'Los cambios se han guardado correctamente.', confirmButtonText:'Aceptar', customClass:{ confirmButton:'btn btn-primary' }, buttonsStyling:false }).then(()=>{ window.location.reload(); }); },
  ...
  confirmAction() {
    ...
    obs.subscribe({ next: res => this.onConfirmSuccess(this.targetUser!,this.confirmKind!,res), error: err => this.onConfirmError(err) });
  }
  ...
  editUser(u: AppUser) { if (this.isSuperAdmin(u)) return; if (this.isCreator(u)||this.isUser(u)) this.openEditModal(u); else if (this.isAdmin(u)) this.openEditAdminModal(u); }
