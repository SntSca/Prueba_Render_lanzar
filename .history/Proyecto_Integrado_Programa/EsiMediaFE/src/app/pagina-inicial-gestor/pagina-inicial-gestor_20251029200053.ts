...
const notAllowedMsg = (userTipo: string | null | undefined, contenidoTipo: string | null | undefined) =>
  `Tu perfil es de tipo ${userTipo || '—'} y este contenido es ${contenidoTipo || '—'}. No puedes modificarlo.`;
...
export class PaginaInicialGestor implements OnInit {
  ...
  canManage = (c: Contenido | null | undefined): boolean => {
    if (!c || !this.userTipoContenido) return false;
    return c.tipo === this.userTipoContenido;
  };

  // --- VER / VISTA (sin cambios: ver está permitido) ---
  abrirVista(c: Contenido) {
  ...
  onToggleVisible(c: Contenido) {
    ...
    if (!this.canManage(c)) {
      void showAlert('Acción no permitida', notAllowedMsg(this.userTipoContenido, c?.tipo), 'error');
      return;
    }
    ...
    c.visible = next;
    this.contenidos.modificar(c.id, { visible: next }, this.userTipoContenido).subscribe({
  ...
  openEdit(c: Contenido) {
    if (!this.userTipoContenido) {
      void showAlert('Error', 'Tu perfil no tiene un tipo de creador asignado.', 'error');
      return;
    }
    if (!this.canManage(c)) {
      void showAlert('Acción no permitida', notAllowedMsg(this.userTipoContenido, c?.tipo), 'error');
  ...
  async saveEdit() {
    ...
    // 🔒 Doble chequeo por si han abierto el modal a la fuerza
    if (!this.canManage(this.editing)) {
      void showAlert('Acción no permitida', notAllowedMsg(this.userTipoContenido, this.editing?.tipo), 'error');
      return;
    }
    ...
    try {
      const updated = await firstValueFrom(
        this.contenidos.modificar(this.editing.id, cleanPayload(this.cambios), this.userTipoContenido)
      );
  ...
  async deleteContenido(c: Contenido) {
    if (!this.userTipoContenido) { void showAlert('Error', 'Tu perfil no tiene un tipo de creador asignado.', 'error'); return; }
