...
export class PaginaInicialGestor implements OnInit {
  ...
  checkAlias() { const alias = trim(this.model.alias); this.aliasAvailable = null; this.aliasChecking = false; if (alias.length < this.aliasMinLength || alias.length > this.aliasMaxLength) { this.aliasAvailable = false; this.cdr.markForCheck(); return; }
  ...
  async saveEdit() {
    ...
    const creatorTipo = (this.userTipoContenido as TipoContenido);
    try {
      const updated = await this.contenidos.modificar(this.editing.id, cleanPayload(this.cambios), creatorTipo).toPromise();
  ...
  async deleteContenido(c: Contenido) {
    ...
    const creatorTipo = (this.userTipoContenido as TipoContenido);
    this.contenidos.eliminar(c.id, creatorTipo).subscribe({
      next: () => {
