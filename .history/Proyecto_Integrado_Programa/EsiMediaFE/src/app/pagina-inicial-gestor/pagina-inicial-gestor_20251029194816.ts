...
export class PaginaInicialGestor implements OnInit {
  ...
  async saveEdit() {
    ...
    try {
      const updated = await firstValueFrom(this.contenidos.modificar(this.editing.id, cleanPayload(this.cambios), this.userTipoContenido));
  ...
  toggleTagEdit(tag: string) {
    if (!this.cambios.tags) this.cambios.tags = [];
