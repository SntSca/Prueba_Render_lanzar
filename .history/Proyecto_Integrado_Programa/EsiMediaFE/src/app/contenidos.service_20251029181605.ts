...
export class ContenidosService {
  ...
  async canStream(opts: HeaderOpts): Promise<void> {
    ...
    if ([301, 302, 303, 307, 308].includes(tiny.status)) return; // redirección válida
    if (!tiny.ok) {
  ...
  /**
   * Resuelve destino de streaming (externo o local) y deja al backend contar
   * la reproducción cuando role='USUARIO'.
   */
  async resolveAndCount(opts: HeaderOpts): Promise<ResolveResult> {
    const headers = this.buildHeaders(opts);
    const url = `${this.BASE}/ReproducirContenido/${encodeURIComponent(opts.id)}`;

    // Intento sin seguir redirección para detectar origen externo
    ...
    if (resManual.status === 0) {
      throw new Error('HTTP0_OPAQUE'); // CORS/opaque
    }
    ...
    // Flujo local: ahora sí seguimos y recuperamos blob reproducible
