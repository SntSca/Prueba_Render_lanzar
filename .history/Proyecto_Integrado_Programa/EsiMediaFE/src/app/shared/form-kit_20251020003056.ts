...
export class Debouncer {
  ...
  set(key: DebounceKey, fn: () => void, ms: number) {
...
export class FormKit {
  ...
  static toDateInput(iso?: string | null) { if (!iso) return ''; return iso.length > 10 ? iso.slice(0, 10) : iso; }
  ...
  static aliasLenError(v: string, min: number, max: number) { const t = this.trim(v); if (!t) return null; return this.within(t, min, max) ? null : `El alias debe tener entre ${min} y ${max} caracteres.`; }
}

export class RoleKit {
  static label(role: 'ADMINISTRADOR'|'USUARIO'|'GESTOR_CONTENIDO'|string) {
