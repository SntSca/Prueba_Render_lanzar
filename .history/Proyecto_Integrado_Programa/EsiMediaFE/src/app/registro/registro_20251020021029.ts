...
const makeDebouncer = () => {
  ...
  const set = (key: DebounceKey, fn: () => void, ms: number) => { const t = store[key]; if (t) clearTimeout(t); store[key] = setTimeout(fn, ms); };
...
export class Registro implements OnInit, OnDestroy {
  ...
  private getAliasError(): string | null {
    if (this.isGestor) return this.aliasRequiredError || this.aliasLenError || (this.aliasUnique === false ? 'El alias ya existe. Elige otro.' : null);
    return (this.alias || '').trim().length > 0 ? this.aliasLenError : null;
  }
  private getGestorExtraError(): string | null { return this.isGestor ? (this.especialidadError || this.tipoContenidoError) : null; }
  private pushPasswordErrors(out: string[]): void {
    if (!this.showPasswordFields) return;
    if (this.pwdIssues.length > 0) out.push('La contraseña no cumple los requisitos.');
    if (this.pwdMismatch) out.push('Las contraseñas no coinciden.');
    const leaks = this.pwnedCount ?? 0;
    if (leaks > 0) out.push(`Esta contraseña aparece en filtraciones públicas ${leaks} veces.`);
  }

  get validationSummary(): string[] {
    const out: string[] = [];
    const add = (msg?: string | null) => { if (msg) out.push(msg); };
    add(this.checkLengths());
    add(this.avatarError);
    add(this.validateAge());
    add(this.fechaInvalida ? 'La fecha no puede ser futura.' : null);
    add(this.emailUnique === false ? 'Este email ya está registrado. Elige otro.' : null);
    add(this.esAltaAdmin ? this.deptoError : null);
    add(this.getAliasError());
    add(this.getGestorExtraError());
    this.pushPasswordErrors(out);
    return out;
  }
  ...
  private async validatePasswordsBlock(form: NgForm): Promise<string | null> {
    const validators = [
      () => (form.invalid || this.fechaInvalida) ? 'Hay campos con errores. Corrígelos y vuelve a intentarlo.' : null,
      () => !this.showPasswordFields ? null : (this.pwdIssues.length > 0 || this.pwdMismatch) ? 'Hay campos con errores. Corrígelos y vuelve a intentarlo.' : null,
      async () => {
  ...
  private async validateAliasBlock(): Promise<string | null> {
    ...
    const validatorsGestor = [
      () => !a ? 'Para Gestor de Contenido, el alias es obligatorio.' : null,
      () => !within(a, ALIAS_MIN, MAX.alias) ? `El alias debe tener entre ${ALIAS_MIN} y ${MAX.alias} caracteres.` : null,
      async () => { await this.ensureUnique('alias', a); return this.aliasUnique !== true ? 'El alias ya existe o no se ha podido verificar. Elige otro.' : null; }
    ] as const;
    const validatorsUser = [
      () => this.esAltaAdmin || isBlank(a) ? null : (!within(a, ALIAS_MIN, MAX.alias) ? `El alias (si lo indicas) debe tener entre ${ALIAS_MIN} y ${MAX.alias} caracteres.` : null),
      async () => { if (this.esAltaAdmin || isBlank(a)) return null; await this.ensureUnique('alias', a); return this.aliasUnique === false ? 'El alias ya existe. Elige otro o deja el campo vacío.' : null; }
    ] as const;
    const list = this.isGestor ? validatorsGestor : validatorsUser;
    for (const v of list) {
      const res = await v();
      if (res) return res;
    }
  ...
  private async preflightValidate(form: NgForm, now: number): Promise<string | null> {
    const validators = [
      () => now - this.lastSubmitAt < 5000 ? 'Espera unos segundos antes de volver a intentarlo.' : null,
      () => this.checkLengths(),
      () => !this.foto ? 'Debes seleccionar una foto de perfil (obligatoria).' : null,
      () => this.validateAge(),
      () => this.fechaInvalida ? 'La fecha no puede ser futura.' : null,
      async () => { await this.ensureUnique('email', this.email); return this.emailUnique === false ? 'Este email ya está registrado. Elige otro.' : null; },
      async () => this.validateAliasBlock(),
      () => this.esAltaAdmin && isBlank(this.departamento) ? 'Para Administrador, el departamento es obligatorio.' : null,
      async () => this.validatePasswordsBlock(form),
      () => {
        if (!this.isGestor) return null;
        this.aliasTouched = this.especialidadTouched = this.tipoContenidoTouched = true;
        if (isBlank(this.especialidad) || !oneOf(this.tipoContenido, ['Audio','Video',''] as const)) return 'Para Gestor de Contenido, especialidad y tipo de contenido (Audio/Video) son obligatorios.';
