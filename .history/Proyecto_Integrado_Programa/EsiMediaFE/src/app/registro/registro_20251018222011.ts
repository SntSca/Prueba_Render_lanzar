...
export class Registro implements OnInit, OnDestroy {
  ...
  // Edad (solo para usuarios normales)
  get ageYears(): number | null {
    if (this.esAltaAdmin || this.esAltaCreador) return null;
  ...
  // Reglas del botón Submit centralizadas
  get extraDisabled(): boolean {
    ...
    const generalInvalidUser =
      ...
      (this.fechaInvalida || this.isUnder4 || !this.termsAccepted);

    const avatarInvalid = !this.selectedAvatar;

    // Alias:
    // - Admin: no se usa
    // - Gestor: obligatorio + disponible
    // - Usuario: OPCIONAL (no bloquea botón)
    const aliasInvalidGestor =
      this.isGestor && (!this.alias || this.alias.trim().length === 0 || this.aliasUnique !== true);

    // Email debe ser "único" en todos los casos (si se pudo comprobar)
  ...
  private async ensureAliasChecked(): Promise<void> {
    if (this.esAltaAdmin) return; // admin no usa alias
  ...
  onAliasChange() {
    if (this.esAltaAdmin) return; // por si acaso
    this.aliasUnique = null;
  ...
  private async ensureEmailChecked(): Promise<void> {
    ...
    // Si el email no cumple el patrón, no consultamos
    ...
    try {
      ...
      this.emailUnique = !!result?.available; // false = ya existe
  ...
  async onSubmit(form: NgForm) {
    ...
    // Edad mínima para usuarios
    if (!this.esAltaCreador && !this.esAltaAdmin) {
      if (this.isUnder4) {
        return this.handleFormError('No se permiten usuarios menores de 4 años.');
      }
    }
    ...
    // - Admin: NO
    // - Gestor: obligatorio + disponible
    // - Usuario: opcional (si viene, comprobar disponibilidad para informar)
    if (this.isGestor) {
      if (!this.alias.trim()) {
      ...
      // Usuario con alias opcional: si lo puso, lo comprobamos para avisar
      await this.ensureAliasChecked();
    ...
    // Gestor: descripcion OPCIONAL; especialidad y tipo requeridos
    if (this.isGestor) {
    ...
    // Alias: admin NO manda; gestor obligatorio; usuario opcional
    if (this.isGestor) {
