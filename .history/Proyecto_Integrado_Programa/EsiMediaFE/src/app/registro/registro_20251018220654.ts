...
export class Registro implements OnInit, OnDestroy {
  ...
  // 👉 NUEVO: centraliza toda la lógica del [disabled] del botón
  get extraDisabled(): boolean {
    ...
    const generalInvalid =
      (!this.esAltaCreador && !this.esAltaAdmin) && (this.fechaInvalida || !this.termsAccepted);

    const avatarInvalid = !this.selectedAvatar;

    const aliasInvalid = !this.esAltaAdmin && (!this.alias || this.aliasUnique !== true);

    const gestorInvalid =
      this.isGestor && (!this.especialidad.trim() || !(this.tipoContenido === 'Audio' || this.tipoContenido === 'Video'));
      // descripción es opcional

    const adminInvalid = this.esAltaAdmin && !this.departamento.trim();

    return passwordsInvalid || generalInvalid || avatarInvalid || aliasInvalid || gestorInvalid || adminInvalid;
  }
  ...
  private async ensureAliasChecked(): Promise<void> {
    if (this.esAltaAdmin) return; // no chequear alias para ADMIN
  ...
  onAliasChange() {
    if (this.esAltaAdmin) return; // por si el input se mostrara
    this.aliasUnique = null;
  ...
  async onSubmit(form: NgForm) {
    ...
    // Alias: obligatorio solo si NO es alta de admin
    if (!this.esAltaAdmin) {
      if (!this.alias.trim())
    ...
    // Gestor: descripción OPCIONAL; se exige especialidad y tipo
    if (this.isGestor) {
