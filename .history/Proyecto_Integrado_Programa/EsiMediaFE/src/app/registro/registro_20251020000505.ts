...
export class Registro implements OnInit, OnDestroy {
  ...
  get validationSummary(): string[] {
    const msgs = [
      this.checkLengths(),
      this.avatarError,
      (!this.esAltaAdmin && !!this.fechaNac && this.isUnder4) ? 'No se permiten registros de menores de 4 años.' : null,
      ...
      this.isGestor ? this.aliasRequiredError : (!!this.alias.trim() ? this.aliasLenError : null),
      this.isGestor ? this.aliasLenError : null,
      this.isGestor && this.aliasUnique === false ? 'El alias ya existe. Elige otro.' : null,
      this.isGestor ? this.especialidadError : null,
      this.isGestor ? this.tipoContenidoError : null,
  ...
  private setDebounce(kind: 'pwned'|'alias'|'email', fn: () => void, ms: number) {
    ...
    const key = map[kind] as 'pwnedDebounce'|'aliasDebounce'|'emailDebounce';
  ...
  private async preflightValidate(form: NgForm, now: number): Promise<string | null> {
    ...
    const len = this.checkLengths(); if (len) return len;
    if (!this.foto) return 'Debes seleccionar una foto de perfil (obligatoria).';
    if (!(this.esAltaCreador || this.esAltaAdmin) && this.fechaNac && this.isUnder4) return 'No se permiten registros de menores de 4 años.';
    if (this.fechaInvalida) return 'La fecha no puede ser futura.';
    await this.ensureEmailChecked(); if (this.emailUnique === false) return 'Este email ya está registrado. Elige otro.';
    const aliasMsg = await this.validateAliasBlock(); if (aliasMsg) return aliasMsg;
    if (this.esAltaAdmin && !this.departamento.trim()) return 'Para Administrador, el departamento es obligatorio.';
    ...
    if (this.isGestor) {
      ...
      if (!this.especialidad.trim() || !(this.tipoContenido === 'Audio' || this.tipoContenido === 'Video')) return 'Para Gestor de Contenido, especialidad y tipo de contenido (Audio/Video) son obligatorios.';
  ...
  private buildPayload(): any {
    const role = this.esAltaAdmin ? 'Administrador' : this.esAltaCreador ? 'Gestor de Contenido' : this.role;
