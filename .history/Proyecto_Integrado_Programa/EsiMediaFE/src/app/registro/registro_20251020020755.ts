...
// Helpers pequeños para mantener baja la complejidad
private getAliasError(): string | null {
  if (this.isGestor) {
    return this.aliasRequiredError
        || this.aliasLenError
        || (this.aliasUnique === false ? 'El alias ya existe. Elige otro.' : null);
  }
  const hasAlias = (this.alias || '').trim().length > 0;
  return hasAlias ? this.aliasLenError : null;
}

  private getGestorExtraError(): string | null {
    return this.isGestor ? (this.especialidadError || this.tipoContenidoError) : null;
  }
  ...
  private buildPayload(): any {
    ...
    if (!(this.esAltaCreador ) && this.fechaNac) base.fechaNac = this.fechaNac;
