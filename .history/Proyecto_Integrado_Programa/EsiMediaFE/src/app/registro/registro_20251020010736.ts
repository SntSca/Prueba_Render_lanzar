...
export class Registro implements OnInit, OnDestroy {
  ...
  private setDebounce(key: DebounceKey, fn: () => void, ms: number) {
    const t = this.debounces[key]; if (t) clearTimeout(t); this.debounces[key] = setTimeout(fn, ms);
  }
  ...
  get aliasLenError(): string | null {
    const a = this.trim(this.alias); if (!a) return null;
  ...
  ngOnDestroy() { Object.values(this.debounces).forEach(t => t && clearTimeout(t)); }

  // ==== UI / pequeños handlers ====
  ...
  // ==== Alias/Email únicos (compacto) ====
  private getNormalizedValue(kind: UniqueKind, raw: string): string {
    return kind === 'email' ? this.lower(raw) : this.trim(raw);
  }
  private shouldSkipUniqueCheck(kind: UniqueKind, value: string): boolean {
    if (kind === 'alias' && this.esAltaAdmin) return true;
    if (this.isBlank(value)) { this.setUniqueStatus(kind, null, ''); return true; }
    if (kind === 'alias' && !this.within(value, this.ALIAS_MIN, this.MAX.alias)) { this.setUniqueStatus(kind, null, ''); return true; }
    if (kind === 'email' && !this.emailPattern.test(value)) { this.setUniqueStatus(kind, null, ''); return true; }
    return false;
  }
  private getAlreadyCheckedValue(kind: UniqueKind): string { return kind === 'alias' ? this.aliasCheckedFor : this.emailCheckedFor; }
  private getUniqueStatus(kind: UniqueKind): boolean | null { return kind === 'alias' ? this.aliasUnique : this.emailUnique; }
  private setCheckingStatus(kind: UniqueKind, status: boolean) { (kind === 'alias') ? (this.isCheckingAlias = status) : (this.isCheckingEmail = status); }
  ...
  private async performUniqueCheck(kind: UniqueKind, value: string): Promise<boolean | null> {
    if (kind === 'alias') {
      ...
      this.setUniqueStatus(kind, !!result?.available, value);
  ...
  private async ensureUnique(kind: UniqueKind, raw: string): Promise<boolean | null> {
    const value = this.getNormalizedValue(kind, raw);
    if (this.shouldSkipUniqueCheck(kind, value)) return null;
    if (value === this.getAlreadyCheckedValue(kind)) return this.getUniqueStatus(kind);
  ...
  onAliasChange() {
    ...
    const v = this.trim(this.alias); if (!v || !this.within(v, this.ALIAS_MIN, this.MAX.alias)) return;
  ...
  // ==== Validación compacta previa al submit ====
  ...
  private checkLengths(): string | null {
    const checks: Array<[boolean, string]> = [
      [this.over(this.nombre, this.MAX.nombre), `El nombre supera ${this.MAX.nombre} caracteres.`],
      [this.over(this.apellidos, this.MAX.apellidos), `Los apellidos superan ${this.MAX.apellidos} caracteres.`],
      [this.over(this.email, this.MAX.email), `El email supera ${this.MAX.email} caracteres.`],
      [this.over(this.departamento, this.MAX.departamento), `El departamento supera ${this.MAX.departamento} caracteres.`],
      [this.over(this.especialidad, this.MAX.especialidad), `La especialidad supera ${this.MAX.especialidad} caracteres.`],
      [this.over(this.descripcion, this.MAX.descripcion), `La descripción supera ${this.MAX.descripcion} caracteres.`],
      [!this.esAltaAdmin && this.over(this.alias, this.MAX.alias), `El alias supera ${this.MAX.alias} caracteres.`],
      [this.isGestor && !!this.alias && !this.within(this.alias, this.ALIAS_MIN, this.MAX.alias), `El alias debe tener entre ${this.ALIAS_MIN} y ${this.MAX.alias} caracteres.`]
    ];
  ...
  private submitByRole(base: any): void {
    const actions: Record<'creador'|'admin'|'usuario', () => void> = {
    ...
    (this.esAltaCreador ? actions.creador : this.esAltaAdmin ? actions.admin : actions.usuario)();
  }
