...
export class Registro implements OnInit, OnDestroy {
  ...
  get fechaInvalida(): boolean { if (this.esAltaAdmin) return false; if (!this.fechaNac) return false; return new Date(this.fechaNac) > new Date(); }
  ...
  get pwdStrengthLabel(): string { if (!this.showPasswordFields) return ''; if (this.pwdScore <= 1) return 'Débil'; if (this.pwdScore <= 3) return 'Media'; return 'Fuerte'; }
  get pwdMismatch(): boolean { if (!this.showPasswordFields) return false; return this.pwd2.length > 0 && this.pwd !== this.pwd2; }
  ...
  get deptoError(): string | null { if (!this.esAltaAdmin) return null; if (!(this.departamento || '').trim()) return 'El departamento es obligatorio.'; return null; }
  get especialidadError(): string | null { if (!this.isGestor) return null; if (!(this.especialidad || '').trim()) return 'Falta la especialidad.'; return null; }
  get tipoContenidoError(): string | null { if (!this.isGestor) return null; if (!(this.tipoContenido === 'Audio' || this.tipoContenido === 'Video')) return 'Selecciona Audio o Video.'; return null; }
  ...
  onRoleChange(val: string) { if (this.rolFijo) return; this.role = val as any; if (val !== 'usuario') this.vip = false; }
  ...
  private buildPayload(): any {
    const base: any = {
      ...
      vip: this.vip,
      role: ((): string => { if (this.esAltaAdmin) return 'Administrador'; if (this.esAltaCreador) return 'Gestor de Contenido'; return this.role; })(),
      foto: this.foto
    };
