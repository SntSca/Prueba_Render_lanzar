...
export class Registro implements OnInit, OnDestroy {
  ...
  if (this.isGestor) {
    ...
    if (a.length < this.ALIAS_MIN || a.length > this.MAX.alias) {
      this.handleFormError(`El alias debe tener entre ${this.ALIAS_MIN} y ${this.MAX.alias} caracteres.`);
      return false;
    }
    await this.ensureAliasChecked();
  ...
  if (!this.esAltaAdmin && !!this.alias.trim()) {
    const a = this.alias.trim();
    if (a.length < this.ALIAS_MIN || a.length > this.MAX.alias) {
      this.handleFormError(`El alias (si lo indicas) debe tener entre ${this.ALIAS_MIN} y ${this.MAX.alias} caracteres.`);
      return false;
    }
    await this.ensureAliasChecked();
