...
export class Registro implements OnInit, OnDestroy {
    ...
    const msgs = [
      ...
      (() => {
        if (this.isGestor) {
          ...
          return this.trim(this.alias) ? this.aliasLenError : null;
        }
      })(),
  ...
  private async ensureUnique(kind: UniqueKind, raw: string): Promise<boolean | null> {
    ...
    const alreadyChecked = this.getAlreadyCheckedValue(kind);
    if (value === alreadyChecked) return this.getUniqueStatus(kind);

    this.setCheckingStatus(kind, true);
    try {
      return await this.performUniqueCheck(kind, value);
    } catch {
      this.setUniqueStatus(kind, null, value);
      return null;
    } finally {
      this.setCheckingStatus(kind, false);
    }
  }
  ...
  private shouldSkipUniqueCheck(kind: UniqueKind, value: string): boolean {
    ...
    if (this.isBlank(value)) {
      this.setUniqueStatus(kind, null, '');
      return true;
    }
  ...
  private getAlreadyCheckedValue(kind: UniqueKind): string {
    return kind === 'alias' ? this.aliasCheckedFor : this.emailCheckedFor;
  }

  private getUniqueStatus(kind: UniqueKind): boolean | null {
    return kind === 'alias' ? this.aliasUnique : this.emailUnique;
  }

  private setCheckingStatus(kind: UniqueKind, status: boolean): void {
    if (kind === 'alias') this.isCheckingAlias = status;
    else this.isCheckingEmail = status;
  }

  private setUniqueStatus(kind: UniqueKind, status: boolean | null, value: string): void {
    if (kind === 'alias') {
      this.aliasUnique = status;
      this.aliasCheckedFor = value;
    } else {
      this.emailUnique = status;
  ...
  private async submitByRole(base: any): Promise<void> {
    ...
    let executeRoleAction;
    if (this.esAltaCreador) {
      executeRoleAction = byRole.creador;
    } else if (this.esAltaAdmin) {
      executeRoleAction = byRole.admin;
    } else {
      executeRoleAction = byRole.usuario;
    }
    executeRoleAction();
  }
