...
export class Registro implements OnInit, OnDestroy {
  ...
  get extraDisabled(): boolean {
    const passwordsInvalid =
      ...
      (this.pwdIssues.length > 0 || this.pwdMismatch || ((this.pwnedCount || 0) > 0));

    const generalInvalidUser =
      (!this.esAltaCreador && !this.esAltaAdmin) &&
      (this.fechaInvalida || this.isUnder4 );

    const avatarInvalid = !this.selectedAvatar;

    const aliasInvalidGestor =
      this.isGestor && (!this.alias || this.alias.trim().length === 0 || this.aliasUnique !== true);
    const emailInUse = this.emailUnique === false;

    const gestorInvalidCampos =
      this.isGestor && (!this.especialidad.trim() ||
      !(this.tipoContenido === 'Audio' || this.tipoContenido === 'Video'));

    const adminInvalid = this.esAltaAdmin && !this.departamento.trim();

    return passwordsInvalid || generalInvalidUser || avatarInvalid || aliasInvalidGestor || emailInUse || gestorInvalidCampos || adminInvalid;
  }
...
private checkLengthsFront(): boolean {
  const msg = this.checkLengths(); // ya tenías este helper
  if (msg) {
    this.handleFormError(msg);
    return false;
  }
  return true;
}

private checkAvatarSelected(): boolean {
  if (!this.foto) {
    this.showAlert('Falta avatar', 'Debes seleccionar una foto de perfil (obligatoria).', 'info');
    return false;
  }
  return true;
}

private checkMinAge(): boolean {
  // Aplica a cualquier rol si hay fechaNac
  if (this.fechaNac && this.isUnder4) {
    this.handleFormError('No se permiten registros de menores de 4 años.');
    return false;
  }
  return true;
}

private async checkEmailAvailability(): Promise<boolean> {
  await this.ensureEmailChecked();
  if (this.emailUnique === false) {
    this.showAlert('Email en uso', 'Este email ya está registrado. Elige otro.', 'error');
    return false;
  }
  return true;
}

/** Alias: Admin no tiene alias; Gestor lo requiere; Usuario es opcional (si viene, se valida). */
private async checkAliasRules(): Promise<boolean> {
  if (this.isGestor) {
    if (!this.alias.trim()) {
      this.handleFormError('Para Gestor de Contenido, el alias es obligatorio.');
      return false;
    }
    await this.ensureAliasChecked();
    if (this.aliasUnique !== true) {
      this.showAlert('Alias no disponible', 'El alias ya existe o no se ha podido verificar. Elige otro.', 'error');
      return false;
    }
    return true;
  }
  ...
  if (!this.esAltaAdmin && !!this.alias.trim()) {
    ...
    if (this.aliasUnique === false) {
      this.showAlert('Alias no disponible', 'El alias ya existe. Elige otro o deja el campo vacío.', 'error');
      return false;
    }
  }
  return true;
...
private checkAdminFields(): boolean {
  if (this.esAltaAdmin && !this.departamento.trim()) {
    this.handleFormError('Para Administrador, el departamento es obligatorio.');
    return false;
  }
  return true;
}
...
/**
 * Emula exactamente la lógica anterior:
 * - formHasErrors = form.invalid || fechaInvalida
 * - Si hay password fields: añade reglas de password y pwned
 * - Si no hay password fields y formHasErrors -> error
 */
private async checkPasswordsBlock(form: NgForm): Promise<boolean> {
  ...
  if (this.showPasswordFields) {
    ...
    if (formHasErrors) {
      this.handleFormError('Hay campos con errores. Corrígelos y vuelve a intentarlo.');
      return false;
    }
...
/** Gestor: especialidad y tipo requeridos; descripción opcional (se limpia trim). */
private checkGestorRequiredFields(): boolean {
    ...
    if (!this.especialidad.trim() || !tipoOk) {
      this.handleFormError('Para Gestor de Contenido, especialidad y tipo de contenido (Audio/Video) son obligatorios.');
      return false;
    }
  ...
  private buildPayload(): any {
    ...
    // Fecha solo para alta estándar
    if (!this.esAltaCreador && !this.esAltaAdmin && this.fechaNac) {
      base.fechaNac = this.fechaNac;
    }

    // Alias: admin NO; gestor obligatorio; usuario opcional si viene
