...
export class Registro implements OnInit, OnDestroy {
  ...
  get fechaInvalida(): boolean { 
    if (this.esAltaAdmin || this.esAltaCreador) { 
      return false; 
    } 
    if (!this.fechaNac) { 
      return false; 
  ...
  get aliasRequiredError(): string | null {
    if (!this.isGestor || !this.rolSeleccionado) return null;
    if (!(this.alias || '').trim() && this.tipoContenidoTouched) return 'El alias es obligatorio para Gestor de Contenido.';
    return null;
  }
  ...
  get especialidadError(): string | null { 
    if (!this.isGestor || !this.rolSeleccionado) return null;
    if (!(this.especialidad || '').trim() && this.tipoContenidoTouched) return 'Falta la especialidad.'; 
  return null; 
  }
  get tipoContenidoError(): string | null { 
    if (this.tipoContenidoTouched && !(this.tipoContenido === 'Audio' || this.tipoContenido === 'Video')) {
      return 'Selecciona Audio o Video.';
    }
    if (!(this.tipoContenido === 'Audio' || this.tipoContenido === 'Video') && this.tipoContenidoTouched) return 'Selecciona Audio o Video.';
    return null;
    }
  ...
  const lenMsg = this.checkLengths();

  const base: Array<[boolean, string | null]> = [
    [!!lenMsg, lenMsg],
    [!!this.avatarError, this.avatarError],
    [!this.esAltaAdmin && !!this.fechaNac && this.isUnder4, 'No se permiten registros de menores de 4 años.'],
    [this.fechaInvalida, 'La fecha no puede ser futura.'],
    [this.emailUnique === false, 'Este email ya está registrado. Elige otro.'],
    [this.esAltaAdmin && !!this.deptoError, this.deptoError],
  ];

  const gestorOrAlias: Array<[boolean, string | null]> = this.isGestor
    ? [
        [!!this.aliasRequiredError, this.aliasRequiredError],
        [!!this.aliasLenError, this.aliasLenError],
        [this.aliasUnique === false, 'El alias ya existe. Elige otro.'],
        [!!this.especialidadError, this.especialidadError],
        [!!this.tipoContenidoError, this.tipoContenidoError],
      ]
    : [
        [!this.esAltaAdmin && !!(this.alias || '').trim() && !!this.aliasLenError, this.aliasLenError],
      ];

  const pwdPairs: Array<[boolean, string | null]> = this.showPasswordFields
    ? [
        [this.pwdIssues.length > 0, 'La contraseña no cumple los requisitos.'],
        [this.pwdMismatch, 'Las contraseñas no coinciden.'],
        [((this.pwnedCount ?? 0) > 0), `Esta contraseña aparece en filtraciones públicas ${this.pwnedCount} veces.`],
      ]
    : [];

  return [...base, ...gestorOrAlias, ...pwdPairs]
    .filter(([cond, msg]) => cond && !!msg)
  ...
  /**
 * SHA-1 hash para consulta de contraseñas en la API Have I Been Pwned.
 * ⚠️ ADVERTENCIA: SHA-1 es considerado débil. 
 * Solo se usa para HIBP y **no para almacenamiento ni autenticación**.
 */
    export async function sha1ForHIBP(text: string): Promise<string> {
  ...
  onRoleChange(val: string) {
    ...
    if(val==='Administrador' || val==='Gestor de Contenido'){
      this.fechaNac='';
    }
  }
  ...
  private checkMinAge(): boolean { 
    if ((this.esAltaCreador || this.esAltaAdmin) || !this.fechaNac) return true;
    if (this.isUnder4) { 
  ...
  private checkGestorRequiredFields(): boolean {
    ...
    this.aliasTouched = true;this.especialidadTouched = true;this.tipoContenidoTouched = true;
