...
public cargarContenidos(): void {
  ...
  async play(c: any) {
    ...
    try {
      const context = this.buildPlaybackContext(c);
      await this.contenidosSvc.canStream(context);

      const result = await this.resolveContent(c, context);
      if (!result) throw new Error('No se obtuvo resultado de reproducción.');

      if (result.kind === 'external') {
        this.handleExternalPlayback(result.url, c, context.role);
        return;
      }

      this.handleInternalPlayback(result, c, context.role);
    } catch (e: any) {
  ...
  private buildPlaybackContext(c: any) {
  ...
  private async resolveContent(c: any, context: any): Promise<ResolveResult | null> {
    try {
      ...
      if (e?.message === 'HTTP0_OPAQUE' && this.isHttpUrl(c?.urlVideo)) {
        ...
        if (String(context.role).toUpperCase() === 'USUARIO') c.reproducciones = this.toNum(c.reproducciones) + 1;
        this.cdr.markForCheck();
        return null;
      }
      throw e;
