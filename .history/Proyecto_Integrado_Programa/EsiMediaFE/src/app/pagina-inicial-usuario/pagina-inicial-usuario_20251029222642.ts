...
public cargarContenidos(): void {
  ...
  async play(c: any) {
    ...
    try {
      ...
      const result = await this.resolvePlayback(c, playbackContext);
  ...
  private async resolvePlayback(c: any, context: any): Promise<ResolveResult | null> {
    try {
  ...
  private handleInternalPlayback(result: ResolveResult, c: any, role: string) {
    ...
    this.playerSrc = result.blobUrl || null;
    this.playingId = c.id;
