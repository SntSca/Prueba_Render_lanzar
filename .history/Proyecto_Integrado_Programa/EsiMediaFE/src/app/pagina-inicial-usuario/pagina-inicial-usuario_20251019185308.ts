...
async guardarCambios() {
  ...
  if (vmsg) return this.handleError(vmsg);
  ...
  const aliasAEnviar = this.getAliasToSend(aliasNuevo);

  if (aliasAEnviar && !(await this.ensureAliasDisponible(aliasAEnviar))) {
    return this.handleError('El alias ya existe. Elige otro.');
  }

  // Continuar con el resto del guardado...
}
...
// Función auxiliar para determinar si enviar alias
private getAliasToSend(aliasNuevo: string): string | undefined {
  if (!this.userAliasActual || !aliasNuevo) return aliasNuevo || undefined;
  return aliasNuevo.localeCompare(this.userAliasActual, undefined, { sensitivity: 'accent' }) === 0
    ? undefined
    : aliasNuevo;
}
