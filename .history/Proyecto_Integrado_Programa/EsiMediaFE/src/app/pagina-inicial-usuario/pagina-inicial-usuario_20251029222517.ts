...
public cargarContenidos(): void {
  ...
  async play(c: any) {
    ...
    try {
      const playbackContext = this.createPlaybackContext(c);
      await this.contenidosSvc.canStream(playbackContext);

      const result = await this.resolveContent(c, playbackContext);
      if (!result) throw new Error('No se obtuvo resultado de reproducción.');

      if (result.kind === 'external') {
        this.handleExternalPlayback(result.url, c, playbackContext.role);
        return;
      }

      this.handleInternalPlayback(result, c, playbackContext.role);
    } catch (e: any) {
      this.showError(e?.message || 'No se pudo reproducir este contenido.');
    } finally {
      this.playingBusy = false;
    }
  }

  private createPlaybackContext(c: any) {
    ...
    return { id: c.id, role, email, vip, fechaNacISO: fecha, ageYears: age };
  }

  private async resolveContent(c: any, context: any): Promise<ResolveResult | null> {
    try {
      return await this.contenidosSvc.resolveAndCount(context);
    } catch (e: any) {
      if (e?.message === 'HTTP0_OPAQUE' && this.isHttpUrl(c?.urlVideo)) {
        this.openExternalStrict(c.urlVideo);
        this.openedExternally = true;
        this.incrementReproductions(c, context.role);
        this.cdr.markForCheck();
        return null;
      }
      throw e;
  ...
  private handleExternalPlayback(url: string, c: any, role: string) {
    this.openExternalStrict(url);
    this.openedExternally = true;
    this.incrementReproductions(c, role);
    this.cdr.markForCheck();
  }

  private handleInternalPlayback(result: ResolveResult, c: any, role: string) {
    this.playerKind = String(c.tipo).toUpperCase() === 'AUDIO' ? 'AUDIO' : 'VIDEO';
    if (result.kind !== 'external' && 'blobUrl' in result) {
      this.playerSrc = result.blobUrl;
    } else {
      throw new Error('Invalid ResolveResult: blobUrl is not available.');
    }
    this.playingId = c.id;
  ...
  private incrementReproductions(c: any, role: string) {
