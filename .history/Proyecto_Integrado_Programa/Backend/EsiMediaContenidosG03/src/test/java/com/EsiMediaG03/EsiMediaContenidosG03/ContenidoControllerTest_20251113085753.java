...
class ContenidoControllerTest {
    ...
    @Test
    @DisplayName("GET /ReproducirContenido/{id} con redirección externa (audio o video) -> 302 Found + Location")
    void stream_redireccionExterna_302() throws Exception {
        ...
        when(ext.externalUrl()).thenReturn("https://cdn.example.com/media.mp4");
        when(ext.mimeType()).thenReturn("video/mp4"); // o "audio/mpeg", no importa para el test

        // Simula que el servicio devuelve un target externo (ya sea audio o video)
        when(contenidoService.resolveStreamingTarget(anyString(), any(), any()))
                .thenReturn(ext);

        // Act
        ResponseEntity<Object> resp = controller.stream(
                "media123", new HttpHeaders(),
                null, null, null, null, null, true
        );

        // Assert
        assertEquals(HttpStatus.FOUND, resp.getStatusCode(), 
            "Debe responder con 302 Found al detectar redirección externa");
        assertEquals("https://cdn.example.com/media.mp4", 
            resp.getHeaders().getFirst(HttpHeaders.LOCATION),
            "Debe incluir la cabecera Location con la URL externa del recurso");
    }
