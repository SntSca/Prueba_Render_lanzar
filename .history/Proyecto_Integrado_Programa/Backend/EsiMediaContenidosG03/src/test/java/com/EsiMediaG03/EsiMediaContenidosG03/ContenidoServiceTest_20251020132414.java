...
class ContenidoServiceTest {
    ...
    class ContenidoModelTest {
        ...
        void setVisible_actualiza_fechaEstado() {
            Contenido c = new Contenido();
            LocalDateTime t0 = c.getFechaEstado(); // se inicializa en el campo
            assertNotNull(t0);

            // simulamos un pequeño paso de tiempo lógico
            c.setVisible(true);
            LocalDateTime t1 = c.getFechaEstado();

            assertNotNull(t1);
            // Fecha nueva diferente y posterior/igual que la inicial
