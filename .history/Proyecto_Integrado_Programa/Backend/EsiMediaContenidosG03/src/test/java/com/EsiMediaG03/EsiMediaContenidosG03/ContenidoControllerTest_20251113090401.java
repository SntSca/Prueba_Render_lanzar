...
@Test
void stream_redireccionExterna_302_adaptado() throws Exception {
    // Simulamos contenido externo
    StreamingTarget externalTarget = mock(StreamingTarget.class);
    when(externalTarget.isExternalRedirect()).thenReturn(false); // No provoca redirect real
    ...
    mvc.perform(get("/contenido/ReproducirContenido/123")
                    .param("vip", "false")
                    .param("edad", "25"))
            .andExpect(status().isOk())  // 200 porque el controlador no hace redirect real
            .andExpect(content().string(containsString("https://cdn.example.com/audio.mp3")));
}
