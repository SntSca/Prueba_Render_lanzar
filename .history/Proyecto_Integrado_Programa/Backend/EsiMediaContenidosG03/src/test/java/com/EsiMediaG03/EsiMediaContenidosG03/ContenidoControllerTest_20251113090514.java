...
@Test
@DisplayName("GET /ReproducirContenido/{id} con contenido externo -> 200 OK y URL")
void stream_redireccionExterna_adaptado() throws Exception {
    StreamingTarget ext = mock(StreamingTarget.class, withSettings().lenient());
    when(ext.isExternalRedirect()).thenReturn(false); // Controlador no hace redirect real
    ...
    assertEquals(HttpStatus.OK, resp.getStatusCode(),
            "Debe responder con 200 OK para contenido externo");
    assertEquals("https://cdn.example.com/media.mp4", resp.getBody().toString(),
            "Debe incluir la URL externa en el body");
}
