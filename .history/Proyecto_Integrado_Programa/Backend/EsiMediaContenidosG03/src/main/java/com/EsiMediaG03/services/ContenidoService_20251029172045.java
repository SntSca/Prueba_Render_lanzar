...
public class ContenidoService {
    ...
    // com.EsiMediaG03.services.ContenidoService  (añade imports)
...
public Contenido modificarContenido(String id,
    ...
    // 1) Permisos
    checkPermisosSobreContenido(actual, requesterEmail, requesterRole, requesterTipo, "modificar");

    // 2) No se permite cambiar tipo ni userEmail (creador)
    // (Como usamos DTO, no viene tipo ni userEmail; si aun así llegan, rechazarías aquí.)

    // 3) Aplicar patch de campos permitidos
    ...
    // 3.b) Campos exclusivos por tipo
    ...
    // 4) Validaciones de negocio usando tus métodos existentes
...
public void eliminarContenido(String id,
    ...
    checkPermisosSobreContenido(actual, requesterEmail, requesterRole, requesterTipo, "eliminar");

    contenidoDAO.deleteById(id);
}

// === Autorización centralizada ===
private void checkPermisosSobreContenido(Contenido c,
                                         String requesterEmail,
                                         String requesterRole,
                                         Contenido.Tipo requesterTipo,
                                         String accionVerbo) {
    boolean isAdmin = requesterRole != null &&
            (requesterRole.equalsIgnoreCase("ADMINISTRADOR") || requesterRole.equalsIgnoreCase("ADMIN"));

    // Si el creador ya no existe: solo ADMIN
    if (c.getUserEmail() == null || c.getUserEmail().isBlank()) {
        if (!isAdmin) {
            throw new ContenidoException("El creador ya no existe; solo un ADMIN puede " + accionVerbo + " este contenido.");
        }
        return;
    }
    ...
    // Debe ser el propio creador
    if (requesterEmail == null || !c.getUserEmail().equalsIgnoreCase(requesterEmail)) {
        throw new ContenidoException("No puedes " + accionVerbo + " contenido de otro creador.");
    }
    ...
    // Tipos deben coincidir exactamente
    if (c.getTipo() != requesterTipo) {
        // Mensajes explícitos para tus reglas
...
private void validarVideo(Contenido contenido) {
    ...
    if (contenido.getResolucion() != null && !contenido.getResolucion().isBlank() &&
        !contenido.getResolucion().matches("(?i)^(720p|1080p|4k)$")) {
        throw new IllegalArgumentException("Resolución de vídeo no válida (solo 720p, 1080p, 4K).");
    }
}
