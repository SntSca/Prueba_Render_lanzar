...
public class UserService {
    ...
    public void eliminarAdmin(String id) {
        ...
        if (u == null) throw new AdminNotFoundException(id);
        if (u.getRole() != User.Role.ADMINISTRADOR) throw new NotAnAdminException();
        User currentAdmin = getCurrentLoggedUser();
        if (currentAdmin != null && currentAdmin.getId().equals(id)) {
            throw new SelfDeletionNotAllowedException("No puedes eliminarte a ti mismo.");
        }

        // 2️⃣ Si el usuario a eliminar es el superadmin → prohibido
        if (isSuperAdmin(u)) {
            throw new SuperAdminProtectionException("eliminar");
        }

        // Si pasa todos los filtros, eliminar
        userDao.deleteById(id);
    }
    private User getCurrentLoggedUser() {
        ...
        if (principal instanceof UserDetails) {
            email = ((UserDetails) principal).getUsername(); // normalmente el email
