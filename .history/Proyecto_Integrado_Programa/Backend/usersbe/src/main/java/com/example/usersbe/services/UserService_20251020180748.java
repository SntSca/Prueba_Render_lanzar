...
public void eliminarAdmin(String id) {
    ...
    // ❌ Impedir eliminar al superadmin
    if (isSuperAdmin(u))
        throw new SuperAdminProtectionException("eliminar");

    // ❌ Impedir que el propio admin se elimine a sí mismo
    // Usando el mismo email que isSuperAdmin compara (superAdminEmail)
    if (superAdminEmail != null &&
        superAdminEmail.trim().equalsIgnoreCase(u.getEmail())) {
        throw new SuperAdminProtectionException("No puedes eliminar tu propia cuenta de administrador.");
    }
