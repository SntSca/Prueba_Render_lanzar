...
public class UserService {
    ...
    private static final String ALIAS_ALREADY_IN_USE = "El alias ya está en uso";
    ...
    public boolean isEmailAvailable(String emailNormalizado) {
        if (emailNormalizado == null || emailNormalizado.trim().isEmpty())
            return false;
    ...
    public void registrar(String nombre, String apellidos, String alias, String email,
        ...
        if (!isAliasAvailable(alias)) {
            throw new InvalidFieldException(ALIAS_ALREADY_IN_USE);
        }
    ...
    public User actualizarCreador(String id, String alias, String nombre,
        ...
        if (alias != null && !alias.isBlank()) {
            ...
            if (byAlias != null && !byAlias.getId().equals(u.getId()))
                throw new DuplicateAliasException(ALIAS_ALREADY_IN_USE);
    ...
    applyNombre(u, nombre);
    applyApellidos(u, apellidos);
    applyAlias(u, alias);
...
private void ensureNotBlocked(User u) {
    if (u.isBlocked()) {
        throw new ForbiddenException("Usuario bloqueado");
    }
}

private void applyNombre(User u, String nombre) {
    if (nombre == null) return;
    String n = nombre.trim();
    if (n.isEmpty())            throw new ValidationException("El nombre no puede estar vacío");
    if (n.length() > 100)       throw new ValidationException("Nombre demasiado largo");
    u.setNombre(n);
}

private void applyApellidos(User u, String apellidos) {
    if (apellidos == null) return;
    String a = apellidos.trim();
    if (a.isEmpty())            throw new ValidationException("Los apellidos no pueden estar vacíos");
    if (a.length() > 100)       throw new ValidationException("Apellidos demasiado largos");
    u.setApellidos(a);
}

private void applyAlias(User u, String alias) {
