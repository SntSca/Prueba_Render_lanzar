...
class UserServiceUsuariosTest {
    ...
    @InjectMocks
    private UserService userService; // ← deja de ser null
    ...
    void actualizarUsuario_ok() {
        ...
        // Ajusta esta línea si tu método devuelve Optional<User>:
        // when(userDao.findByAlias("nuevo")).thenReturn(Optional.empty());
    ...
    void actualizarUsuario_aliasDuplicado() {
        ...
        // Si tu DAO devuelve Optional<User>, usa Optional.of(otro)
        // when(userDao.findByAlias("dup")).thenReturn(Optional.of(otro));
