...
class EmailServiceTest {
    ...
    void send_no_logo() throws Exception {
        // Aseguramos que la ruta no existe
    ...
    @Test
    @DisplayName("sendMail: con logo → addInline")
    void send_with_logo() throws Exception {
        ...
        // Creamos el archivo y directorios
        logo.getParentFile().mkdirs();
        assertTrue(logo.createNewFile() || logo.exists());

        service.sendMail("to@mail.com","sub","<b>html</b>");
        verify(sender, times(2)).createMimeMessage(); // puede ser 1; no fiable según impl, así que comprobamos send
        verify(sender).send(any(MimeMessage.class));
