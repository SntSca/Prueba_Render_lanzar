...
class UserControllerTest {
    ...
    void registrar_usuario_ok() throws Exception {
        ...
        verify(userService).registrar(
                ...
                eq(User.Role.USUARIO), isNull(), isNull(), isNull(), isNull()
        );
    ...
    void forgot_and_reset_password_ok() throws Exception {
        ...
        mvc.perform(post("/users/reset-password")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(om.writeValueAsString(Map.of("token", "tok", "newPassword", "NewPass1!"))))
                .andExpect(status().isOk())
    ...
    void crear_creador_por_admin_ok() throws Exception {
        // 1) Stub robusto: casea exactamente la sobrecarga que usa el controller
        //    registrar(String,String,String,String,String,String,boolean,String,User.Role,String,String,User.TipoContenido)
        doAnswer(inv -> null)
                .when(userService)
                .registrar(
                        anyString(), // nombre
        ...
        // 2) Incluye 'descripcion' en el body (por si tu lógica futura lo comprueba)
        var body = Map.of(
                "nombre","N",
                "apellidos","A",
                "email","c@mail.com",
        ...
        // 3) Verificamos que realmente se llamó al servicio con Role.GESTOR_CONTENIDO
        verify(userService).registrar(
