...
class AuthControllerTest {
    ...
    @Test
    @DisplayName("login: ADMIN requiere MFA → needMfa + startChallenge")
    void login_admin_need_mfa() throws Exception {
        when(limiter.secondsToUnlock(anyString())).thenReturn(0L);
        User admin = new User();
        admin.setId("a1");
        admin.setRole(User.Role.ADMINISTRADOR);
        admin.setMfaEnabled(true);
        admin.setMfaMethod(User.MfaMethod.EMAIL_OTP);

        when(auth.login(anyString(), anyString())).thenReturn(admin);
        doNothing().when(auth).startChallenge(ArgumentMatchers.any(User.class));
        doNothing().when(auth).enableEmailOtp(org.mockito.ArgumentMatchers.any(User.class));

        mvc.perform(post("/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"email\":\"admin@mail.com\",\"password\":\"x\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.needMfa", is(true)))
                .andExpect(jsonPath("$.mfaToken", is("a1")))
                .andExpect(jsonPath("$.mfaMethod", is("EMAIL_OTP")));

        verify(auth).startChallenge(admin);
    }
