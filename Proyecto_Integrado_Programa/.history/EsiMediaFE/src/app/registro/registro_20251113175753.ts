...
get avatarError(): string | null {
  // obligatorio solo para creadores
  if (this.esAltaCreador) {
    return this.selectedAvatar ? null : 'Debes seleccionar un avatar.';
  }
  return null;
}

// En preflightValidate
if (this.esAltaCreador && !this.foto) {
  return 'Debes seleccionar una foto de perfil (obligatoria).';
}
