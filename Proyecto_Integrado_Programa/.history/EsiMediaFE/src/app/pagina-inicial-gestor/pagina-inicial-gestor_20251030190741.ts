...
export class PaginaInicialGestor implements OnInit {
  ...
  onEditClick(c: Contenido) {
    this.withPermission(c, () => {
      ...
      this.listasSeleccionadas = publicas
        .filter(l => Array.isArray(l?.contenidosIds) && l.contenidosIds.some(id => this.idEq(id, c.id)))
        .map(l => l.id);
  ...
  try {
    ...
    // ---- Sync de listas: añadir y quitar en base al diff ----
    const contenidoId = String(this.editing.id);

    // Set de seleccionadas tras editar
    ...
    for (const lista of (this.listasPublicas ?? [])) {
      ...
      const antes = new Set((lista?.contenidosIds ?? []).map(String));

      const estabaAntes = antes.has(contenidoId);
      const estaAhora = selectedSet.has(listaId);

      if (estaAhora && !estabaAntes) {
        await firstValueFrom(this.listasPublicasService.añadirContenido(lista.id, contenidoId));
      } else if (!estaAhora && estabaAntes) {
    ...
    this.loadListasPublicas(); // refresco del listado local
  }
}
