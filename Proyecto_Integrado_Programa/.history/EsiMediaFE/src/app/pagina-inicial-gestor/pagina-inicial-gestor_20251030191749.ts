...
export class PaginaInicialGestor implements OnInit {
  ...
  getListaNombre(id: number): string {
  const lista = this.listasPublicas.find(l => l.id === id);
  return lista ? lista.nombre : 'Desconocido';
}

isListaSelected(id: number): boolean {
  return this.listasSeleccionadas.includes(id);
}

toggleLista(id: number) {
  if (this.isListaSelected(id)) {
    this.listasSeleccionadas = this.listasSeleccionadas.filter(i => i !== id);
  } else {
    this.listasSeleccionadas.push(id);
  }
}
