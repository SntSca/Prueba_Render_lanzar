...
availableTags = {
  ...
  onView(c: Contenido) {
    ...
    .filter(lista => lista.contenidosIds?.includes(this.vistaContenido?.id))
  ...
  onEditClick(c: Contenido) {
    this.withPermission(c, () => {
      ...
      this.listasSeleccionadas = this.listasPublicas.filter(lista => lista.contenidosIds.includes(c.id)).map(lista => lista.id);
    });
  }
  ...
  async saveEdit() {
    ...
    try {
      ...
      const contenidoId = this.editing.id;
      for (const lista of this.listasPublicas) {
        const estaSeleccionada = this.listasSeleccionadas.includes(lista.id);
        const estabaAntes = lista.contenidosIds.includes(contenidoId);
      ...
      this.loadListasPublicas(); 
      const listasQueQuitamos = this.listasPublicas.filter(l =>
  l.contenidosIds.includes(this.editing!.id) &&
  !this.listasSeleccionadas.includes(l.id)
  );

  for (const lista of listasQueQuitamos) {
    this.listasPublicasService.eliminarContenido(lista.id, this.editing.id).subscribe();
  }
  const listasQueAñadimos = this.listasPublicas.filter(l =>
    !l.contenidosIds.includes(this.editing!.id) &&
    this.listasSeleccionadas.includes(l.id)
  );

  for (const lista of listasQueAñadimos) {
    this.listasPublicasService.añadirContenido(lista.id, this.editing.id).subscribe();
  }
    }
  }
  ...
  this.listasPublicasService.listarListas().subscribe({
    ...
    error: (err) => { void showAlert('Error', 'No se pudieron cargar las listas públicas', 'error'); }
  });
}
  ...
  toggleLista(listaId: string) {
    const index = this.listasSeleccionadas.indexOf(listaId);
    if (index > -1) {
      this.listasSeleccionadas.splice(index, 1);
    } else {
      this.listasSeleccionadas.push(listaId);
    }
  }

// Nuevo estado específico para el nombre de la lista a crear
nombreListaNueva: string = '';
savingLista = false;

crearListaPublica() {
  const raw = (this.nombreListaNueva ?? '').trim();
  if (!raw) {
  ...
  const existe = (this.listasPublicas ?? []).some(
    (l: any) => (l?.nombre ?? '').trim().toLowerCase() === raw.toLowerCase()
  );
  if (existe) {
    void showAlert('Error', `Ya existe una lista pública llamada "${raw}".`, 'error');
    return;
  }
  ...
  const nuevaLista = {
    ...
    userEmail: this.userEmail,
    visibilidad: 'PUBLICA', // si tu backend la admite; si no, elimínalo
  };
  ...
  this.http.post('http://localhost:8082/listas', nuevaLista).subscribe({
    next: (listaCreada: any) => {
      void showAlert('¡Éxito!', `La lista "${raw}" se ha creado correctamente.`, 'success');

      // Opcional: actualización optimista (si la API no devuelve la lista, usa nuevaLista)
      if (this.listasPublicas) {
        this.listasPublicas = [listaCreada ?? nuevaLista, ...this.listasPublicas];
      } else {
        this.loadListasPublicas(); // fallback
      }
    ...
    error: (err) => {
      ...
      const msg = status === 409
        ? `Ya existe una lista con el nombre "${raw}".`
        : (apiMsg || 'Error al crear la lista pública.');
      void showAlert('Error', msg, 'error');
    },
    complete: () => {
