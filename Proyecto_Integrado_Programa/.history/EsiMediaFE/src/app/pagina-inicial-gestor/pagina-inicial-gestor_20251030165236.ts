...
export class PaginaInicialGestor implements OnInit {
  ...
  if (!this.listaSeleccionada || !this.listaSeleccionada.trim()) {
    Swal.fire({ title: 'Error', text: 'Debes indicar un nombre para la lista.', icon: 'error' });
    return;
  }
  ...
  const nuevaLista = {
    ...
    userEmail: this.userEmail,
    contenidosIds: [] // empieza vacía
  };

  this.http.post('http://localhost:8080/listas', nuevaLista).subscribe({
    next: () => {
      Swal.fire({
        title: '¡Éxito!',
        text: `La lista "${this.listaSeleccionada}" se ha creado correctamente.`,
        icon: 'success',
