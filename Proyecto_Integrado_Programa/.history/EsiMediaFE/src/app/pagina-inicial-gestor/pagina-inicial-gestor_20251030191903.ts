...
export class PaginaInicialGestor implements OnInit {
  ...
  toggleLista(listaId: any) {
    if (this.isListaSelected(listaId)) {
      this.listasSeleccionadas = this.listasSeleccionadas.filter(i => i !== listaId);
    } else {
  ...
  if (!this.listasSeleccionadas || this.listasSeleccionadas.length === 0) return 'Selecciona listas';
  return this.listasSeleccionadas
    .map(id => this.getListaNombre ? this.getListaNombre(id) : '')
    .filter(Boolean)
    .join(', ');
}
