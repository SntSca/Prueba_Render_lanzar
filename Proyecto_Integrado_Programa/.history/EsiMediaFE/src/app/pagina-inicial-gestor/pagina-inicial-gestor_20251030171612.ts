...
export class PaginaInicialGestor implements OnInit {
  ...
  cargarListasDelContenido() {
    if (!this.cambios?.id) return;
    this.listasSeleccionadas = this.listasPublicas
      .filter(lista => lista.contenidosIds.includes(this.cambios.id))
      .map(lista => lista.id);
  }

  // Método para toggle de listas en el formulario
  toggleLista(id: string) {
    const index = this.listasSeleccionadas.indexOf(id);
    if (index > -1) {
