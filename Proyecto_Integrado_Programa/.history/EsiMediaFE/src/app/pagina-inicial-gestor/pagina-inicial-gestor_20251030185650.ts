...
export class PaginaInicialGestor implements OnInit {
  ...
  listasPublicas: Array<{ id: any; nombre: string; contenidosIds?: any[] }> = []; // FIX: tipado ligero y contenidosIds opcional
  crearListaAbierto = false;
  listasDelContenido: string[] = [];
  cambiosList: any = {};
  listasSeleccionadas: any[] = []; // FIX: siempre array
  ...
  closeEdit() {
    ...
    this.editing = null;
    this.cambios = {};
    this.listasSeleccionadas = []; // FIX: reset seguro
  }
  ...
  onView(c: Contenido) {
    ...
    this.listasDelContenido = (this.listasPublicas ?? [])
      .filter(lista => (lista?.contenidosIds ?? []).some(id => this.idEq(id, c.id))) // FIX null-safe
  ...
  onEditClick(c: Contenido) {
    this.withPermission(c, () => {
      ...
      // FIX: selección inicial de listas robusta y null-safe
  ...
  async saveEdit() {
    ...
    try {
      ...
      // --- FIX: sincronización de listas con Sets y comparaciones por String ---
      ...
      this.loadListasPublicas(); // refresca estado local
    }
  }
  ...
  loadListasPublicas() {
    this.listasPublicasService.listarListas().subscribe({
      next: (listas) => { this.listasPublicas = (listas || []).filter(lista => lista.id !== undefined); this.cdr.markForCheck(); },
  ...
  cerrarCrearLista() { 
    ...
    this.nombreListaNueva = '';       // FIX: reset nombre también
    this.descripcionListaNueva = '';  // FIX
  }

  toggleLista(listaId: any) {
    const idStr = String(listaId);
    const idx = this.listasSeleccionadas.findIndex(x => String(x) === idStr);
    if (idx >= 0) this.listasSeleccionadas.splice(idx, 1);
    else this.listasSeleccionadas.push(listaId);
  }
  ...
  cargarListasDelContenido() {
    ...
    this.listasDelContenido = (this.listasPublicas ?? [])
      .filter(lista => (lista?.contenidosIds ?? []).map(String).includes(idStr)) // FIX null-safe
      .map(lista => lista.nombre);
  }
}
