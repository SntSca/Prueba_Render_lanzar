...
export class PaginaInicialGestor implements OnInit {
  ...
  // id es requerido para evitar errores de tipado
  ...
  get totalPages(): number {
    return Math.max(1, Math.ceil((this.contenidosList?.length || 0) / this.pageSize));
  }

  get pagedContenidos() {
    const start = (this.currentPage - 1) * this.pageSize;
    return (this.contenidosList || []).slice(start, start + this.pageSize);
  }

  // Páginas visibles (ventana centrada: máx 5 botones)
  ...
  // Cada vez que recargas la lista, asegura que la página es válida
  private clampPageAfterLoad() {
