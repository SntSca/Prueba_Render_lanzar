      ...
      .subscribe({
        next: (list) => {
          this.users = (list ?? []).map(u => ({
            ...u,
            // Si ya tiene fotoUrl, la usamos; si tiene foto, construimos la URL completa
            fotoUrl: u.fotoUrl || (u.fotoUrl ? `${window.location.origin}/assets/avatars/${u.fotoUrl}` : null)
          }));
          this.loading = false;
        },
        error: (err) => {
