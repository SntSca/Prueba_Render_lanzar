...
export class PaginaInicialAdmin implements OnInit, OnDestroy {
  readonly FILES_BASE = window.location.origin;
  ...
  ngOnInit(): void {
    ...
    const sessionUser = this.getUserFromLocalStorage();
    ...
    this.aliasCheck$
      .pipe(
        ...
        switchMap(a => {
          ...
          this.aliasAvailable = null;
          return this.api.checkAlias(alias);
        }),
        takeUntil(this.destroy$)
      )
      .subscribe(res => {
        this.aliasChecking = false;
        if (typeof res === 'object' && res && 'available' in res) {
          this.aliasAvailable = !!res.available;
        }
      });
  ...
  get filteredUsers(): AppUser[] {
    ...
    if (this.filtros.tipo) data = data.filter(u => u.role === this.filtros.tipo);
    if (this.filtros.vip !== '') {
      const wantVip = this.filtros.vip === 'si';
      data = data.filter(u => !!u.vip === wantVip);
    }
    switch (this.filtros.ordenar) {
      case 'nombre': data.sort((a, b) => (a.nombre || '').localeCompare(b.nombre || '')); break;
      case 'rol': data.sort((a, b) => (a.role || '').localeCompare(b.role || '')); break;
      case 'vip': data.sort((a, b) => Number(!!b.vip) - Number(!!a.vip)); break;
      case 'fecha':
      default:
        if (data[0]?.createdAt) data.sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || ''));
        break;
    }
    return data;
  }
      ...
      .subscribe({
        next: list => {
          this.users = (list ?? []).map(u => ({
            ...u,
            fotoUrl: u.foto ? `${window.location.origin}/${u.foto}` : (u as any).fotoUrl ?? null
          }));
          this.loading = false;
        },
        error: err => {
  ...
  onAliasChangeCreate(v: string) {
    this.crearAdmin.alias = v;
  ...
  openEditAdminModal(u: AppUser) {
    if (!this.isAdmin(u) || this.isSuperAdmin(u)) return;
    this.editingAdmin = u;
    this.adminEditModel = {
      alias: u.alias ?? '',
      nombre: u.nombre ?? '',
  ...
  saveAdminEdit(form: NgForm) {
    if (!this.editingAdmin || !this.isAdmin(this.editingAdmin) || this.isSuperAdmin(this.editingAdmin)) return;
