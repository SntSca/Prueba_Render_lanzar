...
export class PaginaInicialAdmin implements OnInit, OnDestroy {
  ...
  if (confirmacion) {
    this.auth.logout?.(); // Llama al método de logout si existe
    localStorage.removeItem('user'); // Elimina los datos del usuario guardados
    alert('Sesión cerrada correctamente.');
    this.router.navigateByUrl('/auth/login', { replaceUrl: true }); // Redirige al login
  }
}
