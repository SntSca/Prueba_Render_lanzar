...
crearLista(): void {
  ...
  this.http.post<ListaPublica>(this.LISTAS_BASE, payload).subscribe({
    next: (lista) => {
      ...
      // 🔁 Opcional: si quieres mantener el push local
      const existe = this.listasPublicas.some(l => l.id === lista.id);
      if (!existe) {
        this.misListas.push(lista);
        this.listasPublicas.push(lista);
      }

      // 🔁 Asegurarte de que todo queda sincronizado con el backend
      this.cargarListasPublicas();
      ...
      // Si quieres, dejar seleccionada la nueva lista automáticamente:
      // this.filtrosContenido.listaId = lista.id;
