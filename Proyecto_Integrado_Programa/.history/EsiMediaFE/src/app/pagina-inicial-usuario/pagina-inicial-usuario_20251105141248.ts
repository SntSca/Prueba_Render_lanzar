...
export class PaginaInicialUsuario implements OnInit {
  ...
  goMisFavoritos(): void {
    ...
    if (this.onlyFavsView) {
      Swal.fire({ icon: 'info', title: 'Vista cambiada', text: 'Mostrando todos los contenidos.' });
    ...
    Swal.fire({ icon: 'info', title: 'Mis Favoritos', text: 'Mostrando tus contenidos favoritos.' });
  ...
  onRated(id: string, _resumen: any) {
  ...
  // === APERTURA/CIERRE DE TARJETA (POP-CARD) ===
  openedCardId: string | null = null;

  isOpen(c: any): boolean {
    return this.openedCardId === c.id;
  }

  // Abrir la tarjeta con click en el article. Se ignoran clicks internos por el stopPropagation del template.
  openCard(c: any, ev?: MouseEvent) {
    ev?.stopPropagation();
    if (this.openedCardId === c.id) return; // ya está abierta
    this.openedCardId = c.id;
    this.cdr.markForCheck();
  }

  // Cerrar tarjeta desde overlay o por ESC
  closeCard(c: any) {
    if (this.openedCardId === c.id) {
      this.openedCardId = null;
      this.cdr.markForCheck();
    }
  }

  // Cierre global con tecla ESC
