...
export class PaginaInicialUsuario implements OnInit {
  ...
  this.showCrearListaPanel = !this.showCrearListaPanel;
...
private cargarListasPublicas(): void {
  ...
  const payload: Partial<ListaPublica> = {
    nombre,
    descripcion,
    // el backend realmente usa userEmail, pero mantenemos emailUsuario
    emailUsuario: this.userEmail,
    userEmail: this.userEmail as any, // por si el backend mapea ese campo
    publica: false,                   // 👈 SIEMPRE PRIVADA
    contenidosIds: []
  };
  ...
  this.http.post<ListaPublica>(this.LISTAS_BASE, payload).subscribe({
    next: (lista) => {
      ...
      // opcional: cerrar el desplegable al crear
      this.showCrearListaPanel = false;
