...
anadirContenidoALista(c: Contenido): void {
  if (!c?.id || this.readOnly) return;

  const listaId = this.selectedListaPorContenido[c.id];
  if (!listaId) return;
  ...
  // ⚠ AJUSTA ESTA URL AL ENDPOINT QUE TENGAS EN EL BACKEND
  // Ejemplo: POST /listas/{listaId}/contenidos/{contenidoId}
  ...
  this.http.post<ListaPublica>(url, {}).subscribe({
    next: (listaActualizada) => {
      ...
      // Refrescamos en memoria la lista actualizada, si viene en la respuesta
