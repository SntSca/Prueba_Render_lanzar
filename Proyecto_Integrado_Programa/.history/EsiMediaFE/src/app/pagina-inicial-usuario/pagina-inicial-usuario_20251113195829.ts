...
export class PaginaInicialUsuario implements OnInit {
  ...
  private readonly LISTAS_BASE = 'http://localhost:8082/listas';
  ...
  // 🔹🔹 FILTRO POR LISTA PÚBLICA 🔹🔹
  const listaId = String(f.listaId || '').trim();
  if (listaId) {
    const lista = this.listasPublicas.find(l => l.id === listaId);

    // Si en tu backend `contenidos` es un array de IDs:
    const idsLista = new Set(lista?.contenidos ?? []);

    // Si fuera un array de objetos Contenido, sería:
    // const idsLista = new Set((lista?.contenidos ?? []).map((c: any) => c.id));

    working = working.filter(c => idsLista.has(c.id));
  }
  // 🔹🔹 FIN FILTRO POR LISTA 🔹🔹
  ...
  private cargarListasPublicas(): void {
    this.http.get<ListaPublica[]>(`${this.LISTAS_BASE}/publicas`).subscribe({
      next: (listas) => {
