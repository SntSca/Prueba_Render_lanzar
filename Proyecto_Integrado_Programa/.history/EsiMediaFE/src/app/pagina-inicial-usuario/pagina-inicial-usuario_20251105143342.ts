...
export class PaginaInicialUsuario implements OnInit {
  ...
  // Estado del filtro (Todos / Favoritos / Historial)
filterMode: 'todos' | 'favoritos' | 'historial' = 'todos';

// Reacciona al cambio de desplegable
onFilterChange(): void {
  // Si pide favoritos y aún no cargamos IDs, los cargamos y al terminar aplicamos
  if (this.filterMode === 'favoritos' && !this.favsLoaded) {
    this.loadFavoritos();
    // Aplicaremos el filtro en cargarContenidos() o cuando llegue setFavIds.
  }
  // Reaplica el filtro sobre el catálogo actual
  this.applyFilter();
}

// Aplica el filtro actual sobre this.contenidos o sobre el backup cargado
private applyFilter(): void {
  // Si no hay catálogo base guardado, usa el actual como base
  ...
  if (this.filterMode === 'favoritos') {
    if (!this.favsLoaded) {
      // Aún no están listos los favoritos: no filtramos para no vaciar la vista
    ...
    // Versión simple: muestra los que tengan reproducciones > 0 (cliente)
    // Si en tu backend tienes endpoint de historial, cámbialo por esa llamada.
...
// Sobrescribe setFavIds para re-aplicar filtro cuando terminen de cargar los favoritos
private setFavIds(ids: string[] | null | undefined) {
  this.favIds = new Set((ids ?? []).filter(Boolean));
  this.favsLoaded = true;
  // Si estamos en modo favoritos, re-aplicamos inmediatamente
