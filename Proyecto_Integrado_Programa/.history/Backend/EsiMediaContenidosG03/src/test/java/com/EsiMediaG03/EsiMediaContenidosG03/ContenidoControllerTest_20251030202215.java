...
@MockitoSettings(strictness = Strictness.LENIENT) // evita PotentialStubbingProblem con argumentos distintos
class ContenidoControllerTest {
    ...
    /** Necesita mockito-inline o mock-maker-inline para mockear métodos final/record */
    ...
    @Test
    @DisplayName("GET /ReproducirContenido/{id} sin Range -> 200 OK, headers y body completo")
    void stream_sinRange_ok() throws Exception {
        ...
        assertEquals(26L, resp.getHeaders().getContentLength());

        assertTrue(resp.getBody() instanceof InputStreamResource);
        try (InputStream is = ((InputStreamResource) resp.getBody()).getInputStream()) {
            assertArrayEquals(data, readAll(is));
        }
    ...
    @Test
    @DisplayName("GET /ReproducirContenido/{id} con Range válido -> 206 Partial Content y Content-Range correcto")
    void stream_conRange_206() throws Exception {
        ...
        assertEquals("application/octet-stream", resp.getHeaders().getFirst(HttpHeaders.CONTENT_TYPE));
        assertEquals("bytes", resp.getHeaders().getFirst(HttpHeaders.ACCEPT_RANGES));
        assertEquals("bytes 0-9/26", resp.getHeaders().getFirst(HttpHeaders.CONTENT_RANGE));
        assertEquals(10L, resp.getHeaders().getContentLength());

        assertTrue(resp.getBody() instanceof InputStreamResource);
        try (InputStream is = ((InputStreamResource) resp.getBody()).getInputStream()) {
            byte[] chunk = readAll(is);
            assertEquals(10, chunk.length);
            assertArrayEquals("abcdefghij".getBytes(), chunk);
        }
    }
